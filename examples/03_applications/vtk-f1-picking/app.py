...
app = App('F1 Probing', root=__file__, backend='vtk')
...
def update_picking_mode():
    ...
    if mode is None:
        ...
        app.set('interactorSettings', VIEW_SELECT if mode == 'select' else VIEW_INTERACT)
    ...
    if app.dirty('pickData') and data and data['representationId'] == 'f1':
        ...
        if idx > -1:
            ...
            if 'height' in cone_state:
                ...
                for i in range(3):
                    new_center[i] -= 0.5 * cone_state['height'] * \
                        cone_state['direction'][i]
            ...
            if len(messages):
                ...
                app.set('tooltipStyle', {'position': 'absolute', 'left': f'{x + 10}px',
                                         'bottom': f'{y + 10}px', 'zIndex': 10, 'pointerEvents': 'none'})
